...
def get_instruction_q_target_json(question, target_info):
    json_format = '{"answer": "YES"} もしくは {"answer":  NO"}'
    
    system_role = f"""
Aあなたは優秀なE-commerce店舗の店長です。
    """
...
- 回答はJSON形式で、'answer'というキーに対して、"YES"もしくは"NO"とします。
 - この2パターンのどれかになります。{json_format}
...
def _get_instruction_q_target_json(question, target_info):
    json_format = '{"answer": "YES"} もしくは {"answer":  NO"}'
...
def main (tgt_file):
    ...
    #with tgt_file.open(newline="", encoding="utf-8", errors="replace") as f:
        ...
        for row in reader:
            ...
            system_role, instruction = get_instruction_q_target_json(question, target_info)
            #print(f'===>{system_role}')
            #print(f'--->{instruction}')
            #sys.exit()
